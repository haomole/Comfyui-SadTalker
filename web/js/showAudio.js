...
function previewAudio (node, file) {
  ...
  previewWidget.audioEl.style['height'] = "30px"
  ...
  let params = { "filename": file, "type": "input", }
...
function audioUpload (node, inputName, inputData, app) {
  ...
  */
  ...
  uploadWidget = node.addWidget("button", "upload", "Audio", () => {
    fileInput.click();
  });
...
app.registerExtension({
  name: "SadTalker.ShowAudio",
  async beforeRegisterNodeDef (nodeType, nodeData, app) {
    if (nodeData?.name == "ShowAudio") {
